...
function Inspector(controller) {
	...
	****/
	...
	addNodeToView = function (tor, parent, sub, i) {
		var pp = sub;
		var fullPath = sub.name;
		while (pp.parent && typeof(pp.parent.name) != "undefined") {
			fullPath = pp.parent.name + "/" + fullPath;
			pp = pp.parent;
		}
		var row;
		row = new FileRow(tor, sub.depth, sub.name, sub.file_indices, i%2, fullPath);
